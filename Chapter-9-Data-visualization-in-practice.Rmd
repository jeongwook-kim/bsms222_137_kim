...
$2^2, 2^3, 2^4, \dots$  or  $10^2, 10^3, \dots$  are easy to compute in our heads, the same is not true for  $e^2, e^3, \dots$ , so the scale is not intuitive or easy to interpret.
...
p <- gapminder %>% 
  ...
  scale_x_continuous(trans = "log2") 
p  + geom_density_ridges(bandwidth = 0.648) 
...
```{r}
p + geom_density_ridges(jittered_points = TRUE,bandwidth = 0.648)
...
gapminder %>% 
  ...
  ggplot(aes(dollars_per_day, group)) + 
  scale_x_continuous(trans = "log2") + 
  geom_density_ridges(adjust = 1.5, bandwidth = 0.726) +
  facet_grid(. ~ year)
```
