...
```{r paged.print=FALSE}
d%>%
  group_by(Chr,Pos_hg19)%>%
  count%>%
  mutate(position=paste("Chr",Chr,":",Pos_hg19))
...
```{r paged.print=FALSE}
c.e<-d%>%
  filter(Effect%in%c("Nonsense","Frameshift","Missense"))%>%
  group_by(c.DNA)%>%
  count(  )
c.e
```
...
```{r}
ggplot(c.e,aes(c.DNA,n))
```

https://ars.els-cdn.com/content/image/1-s2.0-S1525157810603546-gr2.gif
...
d%>% 
  ...
  mutate( protein_num= gsub('[^0-9]', '',p.Protein))
