...
```{r fy}
file.copy(fullpath, "murder2.csv", overwrite = TRUE)
...
```
```{r paged.print=FALSE}
#read_csv(file.path(path,files[1])) : .xls
...
#read_csv(file.path(path,files[8])) : pdf
...
```{r}
read_lines(file.path(path,files[7]),n_max = 1)
```
