---
title: 'Tutorial_Human_Genome_Annotation'
output:
  html_document:
    df_print: paged
---
...
The file is the GFT file format^[https://asia.ensembl.org/info/website/upload/gff.html], which you will find most commonly in gene annotation. Please read the file format thoroughly in the link above.
...
Here I introduce the package . Let's install the package first. 
...
One thing you can find is that there is no columns in the data frame. Let's match which information is provided in columns. You can find the instruction page in the website^[https://www.gencodegenes.org/pages/data_format.html]. 
...
1. Havana: Manual gene annotation^[https://asia.ensembl.org/info/genome/genebuild/manual_havana.html]
2. Ensembl: Automatic gene annotation^[https://asia.ensembl.org/info/genome/genebuild/automatic_coding.html]
...
-https://www.biostars.org/p/61823/ 
-https://www.biostars.org/p/209854/ Also fun to have a review for -https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5863539/
-https://asia.ensembl.org/info/genome/genebuild/transcript_quality_tags.html#tsl
...
```{r}
d %>% mutate(gene_id2 = do.call(rbind.data.frame, strsplit(gene_id, '.', fixed = T))[[1]]) %>% group_by(gene_id2, type) %>% 
...
```{r}
d %>% group_by(seqnames, gene_type, type) %>% 
  filter(type == "transcript") %>%
  count()
```
...
filter(seqnames!="chrM")%>%
  mutate(type2=ifelse(seqnames%in%c("chrX", "chrY"), "sex", "autosomal")) %>% group_by(type2)%>%
  summarize(distribution = quantile(width, seq(0, 1, 0.25)))
```
...
```{r}
gene <- data.frame(gene_id = d$gene_id, gene_name = d$gene_name, hgnc_id = d$hgnc_id, gene_type =d$gene_type, chromosome = d$seqnames, start = d$start, end = d$end, strand = d$strand, isHgnc = ifelse(is.na(d$hgnc_id) , 0, 1), isCCDS = ifelse(is.na(d$ccdsid), 0, 1))
```

2. count the number of hgnc by gene biotypes.
```{r}
gene%>%
  group_by(gene_type)%>%
  filter(isHgnc==1)%>%
  count()
```
3.Please count the number of hgnc by level. Please note that level in this question is not TSL. Please find information in this : 1 (verified loci), 2 (manually annotated loci), 3 (automatically annotated loci).
```{r}
gene%>%
  mutate(level=d$level)%>%
  group_by(level)%>%
  filter(isHgnc==1)%>%
  count()
...
filter(gene_type==c("protein_coding", "lncRNA"))%>%
  mutate(type3=ifelse(gene_type=="protein_coding", "protein_coding", "lncRNA"))%>%
  group_by(type3)%>% 
  summarize(distribution = quantile(width,seq(0,1,0.25)))
...
```{r}
d%>% mutate(gene_id2 = do.call(rbind.data.frame, strsplit(gene_id, '.', fixed = T))[[1]]) %>%
  group_by(seqnames,gene_id2)%>%
...
d%>%
filter(gene_type==c("protein_coding", "lncRNA"))%>%
  mutate(type3=ifelse(gene_type=="protein_coding", "protein_coding", "long noncoding"))%>%
  group_by(type3)%>% 
  count(gene_type) %>% 
  summarize(Mean=mean(n),Median=median(n))
```