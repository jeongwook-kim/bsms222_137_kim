...
You can download the data from . Please save this file to the working folder. Then, load the objects to your workspace.
```{r}
#load('data.TCGA_LUAD_LUSC.gene_expression.Rdata')
...
```{r}
# Q1.Turning implicit missing values into explicit missing values. Adding the row, we can compare dataframes each other correctly (in this plot, about the TR.) and see them comfortably.
# Q2.Wrapping a 1d sequence of panels into 2d. This is generally a better use of screen space than facet_grid() because most displays are roughly rectangular.
```
...
ggplot( aes(age_at_diagnosis)) + geom_histogram(bins=100)
```
...
d_luad%>%
  filter(!is.na(age_at_diagnosis))%>%
ggplot( aes(age_at_diagnosis/365)) + geom_histogram(bins=100)
```
...
d_luad%>%
  filter(!is.na(age_at_diagnosis))%>%
ggplot(., aes(age_at_diagnosis)) + geom_density()
...
ggplot(., aes(gender, age_at_diagnosis/365)) + 
  geom_violin(aes(fill=gender))  + 
  geom_boxplot(fill='white') + 
  geom_dotplot(binaxis='y', stackdir='center', dotsize=1, alpha=0.5,binwidth = 1.6)
```
...
p <-bind_rows(d_luad %>% 
            mutate(type='luad') %>%
            select(type, shortLetterCode, gender), 
          d_lusc %>% 
              
            mutate(type='lusc') %>%
            select(type, shortLetterCode, gender)) %>% 
  dplyr::count(type, shortLetterCode, gender) %>% 
  complete(gender, type, shortLetterCode, fill = list(n = 0)) %>%
  ggplot(., aes(shortLetterCode, n, fill=gender)) + 
    labs(title = 'Cancer type by shortLetterCode with gender',
       x = 'shortLetterCode', y='Number of samples')+
  geom_bar(stat="identity", position=position_dodge()) + 
  facet_wrap(~type) 
p

# save to file
ggsave('plot.TCGA.histogram_SLC_gender.20211027.pdf', p, width = 16, height = 9)
```


## 3.3. Tumor stages of lung cancers
First information you might want to explore is staging cancers. Different types of staging systems are used for different types of cancer. You can read further information in general staging rules^[https://www.nhs.uk/common-health-questions/operations-tests-and-procedures/what-do-cancer-stages-and-grades-mean/], or specifics in lung cancers (d.g. stage IA^[https://www.cancer.gov/publications/dictionaries/cancer-terms/def/stage-ia-non-small-cell-lung-cancer]).
...
bind_rows(d_luad %>% 
  ...
  mutate(tumor_stage = factor(ajcc_pathologic_stage, levels=rev(unique(ajcc_pathologic_stage)))) %>%
...
```{r}
bind_rows(d_luad %>% 
            filter(shortLetterCode == 'TP'&!is.na(age_at_diagnosis)) %>% 
            mutate(type='luad') %>%
