...
```{r}
dat1<-us_contagious_diseases%>%
  filter(!state%in%c("Hawaii","Alaska")&disease=="Smallpox"&!weeks_reporting<10)%>%mutate(rate=count/population*10000*52/weeks_reporting)%>%
  mutate(state=reorder(state,rate))
...
avg <- us_contagious_diseases %>%
  ...
  summarize(us_rate = sum(count*52/weeks_reporting, na.rm=TRUE)/sum(population, na.rm=TRUE)*10000)
...
us_contagious_diseases%>%
  ...
  summarize(rate=sum(count, na.rm=TRUE)/sum(population, na.rm=TRUE)*10000*52/weeks_reporting)%>%
...
us_contagious_diseases %>%
  ...
  summarize(rate=sum(count*52/weeks_reporting,na.rm =T )/sum(population)*10000) %>%
  ggplot(aes(year, rate,color=disease)) + 
  geom_line()
```
